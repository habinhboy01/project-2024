...
function custom_tour()
{
    ...
    register_post_type('tour', [
        ...
        'show_in_rest' => true,
        'rewrite' => ['slug' => 'tour'],
        'taxonomies' => ['category-tour', 'location-tour'],
        'labels' => [
    ...
    register_taxonomy('location-tour', ['tour'], [
        'label' => __('Location tour', 'txtdomain'),
        'hierarchical' => true,
        'rewrite' => ['slug' => 'location-tour'],
        'show_admin_column' => true,
        'show_in_rest' => true,
        'labels' => [
            'singular_name' => __('location-tour', 'txtdomain'),
            'all_items' => __('All location-tour', 'txtdomain'),
            'edit_item' => __('Edit location-tour', 'txtdomain'),
            'view_item' => __('View location-tour', 'txtdomain'),
            'update_item' => __('Update location-tour', 'txtdomain'),
            'add_new_item' => __('Add New location tour', 'txtdomain'),
            'new_item_name' => __('New location-tour Name', 'txtdomain'),
            'search_items' => __('Search location-tour', 'txtdomain'),
            'parent_item' => __('Parent location tour', 'txtdomain'),
            'parent_item_colon' => __('Parent location-tour:', 'txtdomain'),
            'not_found' => __('No location-tour found', 'txtdomain'),
        ],
    ]);
    register_taxonomy_for_object_type('location-tour', 'tour');
}
...
function location_tour()
{
    $cats = get_the_terms(get_the_ID(), 'location-tour');
    foreach ($cats as $cat) :
        if ($cat->slug != 'location-tour') :
            // echo '<a href="' . get_term_link($cat->slug, 'career') . '">';
            echo $cat->name;
            // echo '</a>';
        endif;
