...
function csw_search_form_fc(){
    ...
    <form role="search" method="get" class="form-room searchform" action="<?php echo get_home_url() ?>">
        <input type="hidden" class="input-career" placeholder="Từ khoá" value="" name="s">

        <div class="info_search">
            <label for="desti">Điểm đến:</label> <br>
            <select class="input-career2" name="category-location" id="desti">
        ...
        <div class="info_search">
            <label for="start">Ngày đến:</label><br>
            <input type="date" id="start" name="trip-start" value=""/>
        </div>
        
        
        <div class="info_search">
            <label for="end">Ngày đi:</label><br>
            <input type="date" id="end" name="trip-end" value=""/>
        </div>
        
        <div class="info_search">
            <label for="tentacles">Số lượng: </label> <br>

            <div class="input-quantity">
                <input type="number" id="qua-person" name="tentacles" min="1" max="2" />
            </div>
...
add_shortcode( 'csw_search_form_2', 'csw_search_form_fc_2' );
function csw_search_form_fc_2(){
    ob_start();
    ?>
    ...
    <form role="search" method="get" class="form-room searchform" action="<?php echo get_home_url() ?>">
        ...
        <div class="info_search">
            <label for="desti">Điểm đến:</label> <br>
            <select class="input-career2" name="location-tour" id="desti">
                <option value="">Chọn Địa Điểm</option>

                <?php
                   $args = array(                   
                        'type' => 'tour',
                        'taxonomy' => 'location-tour',
                        'parent' => 0,
                        'hide_empty' => false,
                    );
...
add_shortcode( 'csw_search_form_3', 'csw_search_form_fc_3' );
function csw_search_form_fc_3(){
    ob_start();
    ?>
    ...
    <form role="search" method="get" class="form-room searchform" action="<?php echo get_home_url() ?>">
        ...
        <div class="info_search">
            <label for="desti">Điểm đến:</label> <br>
            <select class="input-career2" name="location-golf" id="desti">
                <option value="">Chọn Địa Điểm</option>

                <?php
                   $args = array(                   
                        'type' => 'tour',
                        'taxonomy' => 'location-golf',
                        'parent' => 0,
                        'hide_empty' => false,
                    );
...
function custom_room_search_filter($query) {
    if (!is_admin() && $query->is_main_query() && is_search()) {
        $location = isset($_GET['category-location']) ? sanitize_text_field($_GET['category-location']) : '';
...
add_action('pre_get_posts', 'custom_room_search_filter');
